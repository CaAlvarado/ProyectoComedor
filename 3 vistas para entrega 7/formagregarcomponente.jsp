	...
	<div class="container">
		...
		<div class="row">
		  ...
		  <br>
			...
			<div >
				<c:url var="url" value="guardarComponente"/>
				
				<c:if test="${actualizar}">
				<c:url var="url" value="/actualizarComponente"/> 
				</c:if>
				
				
				<form:form id="componenteRegisterForm" modelAttribute="componente" method="post" action="${url}">
					<form:hidden path="idComponente"  value="${componente.idComponente}" />
				  <div class="form-group">
				    <label>Nombre del componente del plato</label>
				    <form:input path="nombre" value="${componente.nombre}" cssClass="form-control" required="required"/> <!-- Aca no les cambie los nombre de los path o del value porque si no, no me andaban para probarlo, hay que cambiarlo dependiendo de lo que pongamos en el controlador -->
				  ...
				  <div><h4>Elija el boton correspondiente dependiendo</h4></div>
				  <div><h4>el tipo de componente que usted desea agregar</h4></div>
				  <div><h4>En cualquier caso usted podra luego eliminarlo o editarlo</h4></div>
				  
				  <!-- ACA ESTAN AGREGADOS LOS BOTONES COMPA, TE DEJE EL BOTON VIEJO COMENTADO POR SI NECESITAS ALGUNA INFORMACION DE ESE, DESPUES QUE LE SAQUES LO QUE NECESITAS BORRALO NOMAS -->
				
				
				
					<button onclick="return submitusuarioForm();" type="submit" class="btn btn-default btn-lg btn-block">Entrada</button>
					
					<button onclick="return submitusuarioForm();" type="submit" class="btn btn-default btn-lg btn-block">Principal</button>
					
					<button onclick="return submitusuarioForm();" type="submit" class="btn btn-default btn-lg btn-block">Bebida</button>
					
					<button onclick="return submitusuarioForm();" type="submit" class="btn btn-default btn-lg btn-block">Postre</button>
					 
					 
					 
					<!--	BOTON VIEJO COMENTADO   
